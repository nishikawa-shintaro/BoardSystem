	...
	<div class="main-contents">
		...
		<form action="useredit" method="post">
			<label for="editName">${editUser.name}さんの編集</label><br /> <input
				...
				name="loginId" id="loginId" value="${editUser.loginId }" /><br /> <label
				...
				value="${editUser.name }" /><br /> <label for="branchId">支店名</label><br>
			<select name="branchId">
				<c:forEach items="${branches}" var="branch">
					<option value="${branch.id}"
						<c:if test="${ branch.id == editUser.branchId }">selected</c:if>>
						<c:out value="${branch.name}" />
					</option>
				</c:forEach>
			</select> <br>
			<label for="possitionId">部署・役職</label><br> <select
				name="possitionId">
				<c:forEach items="${possitions}" var="possition">
					<option value="${possition.id}"
						<c:if test="${ possition.id == editUser.possitionId }">selected </c:if>>
